...
for a 2-pool model with exchange.
The variable names all refer to "myelin"
but the methods are general for any 2-pool model.
=#

export bssfp

using BlochSim: Position, SpinMC, excite, freeprecess, duration
...
# In:
- `τ_fs_ms` residence time for exchange
  from fast T2 pool (myelin water) to slow T2 pool (non-myelin water) (ms)
...
- `Δf_myelin_Hz` # additional off-resonance value experienced
  only by fast (myelin) water (Hz)
